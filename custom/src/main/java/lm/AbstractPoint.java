...
public abstract class AbstractPoint {
	...
	public AbstractPoint(int id, TrackingBox trackingBox) {
		...
		isActive = true;
		pointMotionListeners = new CopyOnWriteArrayList<PointMotionListener>();
		clickZone = EnumSet.noneOf(Zone.class);
		position = new Vector();
		pointListeners = new CopyOnWriteArrayList<PointListener>();
		zone = new Vector();
	...
	protected void updateEvents() {
		pointEvent = new PointEvent(this, zones, prevZones, clickZone);
		fireUpdateZone();
		fireUpdatePosition();
		fireUpdateDragg();
	}
	...
	private boolean updateZone() {
		if (!isActive) {
			...
			bufferedZone = EnumSet.of(getZone(zone.getX(), Zone.RIGHT, Zone.LEFT),
					getZone(zone.getY(), Zone.DOWN, Zone.UP), getZone(zone.getZ(), Zone.BACK, Zone.FRONT));
		}
